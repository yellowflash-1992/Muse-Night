const __vite__mapDeps = (
  i,
  m = __vite__mapDeps,
  d = m.f ||
    (m.f = [
      "assets/_id-Cj13oehp.js",
      "assets/createLucideIcon-BnNKfcXr.js",
      "assets/arrow-left-CaFqYGEK.js",
      "assets/book-marked-CbT39qVx.js",
      "assets/circle-check-CXPeO8Jn.js",
      "assets/send-DWuMIr-w.js",
      "assets/sparkles-arFjGQUh.js",
      "assets/literature-j2lTA6at.js",
    ]),
) => i.map((i) => d[i]);
import { n as e, r as t, t as n } from "./preload-helper-BC4arR-S.js";
import { o as r } from "./literature-j2lTA6at.js";
var i = t(`/books/$id`)({
  head: ({ params: e }) => {
    let t = r(e.id);
    return {
      meta: [
        { title: `${t ? t.title : `Chapbook`} — Muse Books` },
        {
          name: `description`,
          content: t
            ? `${t.title} by ${t.author}. ${t.edition}.`
            : `Hand-bound chapbook by Muse Books.`,
        },
      ],
    };
  },
  component: e(
    () => n(() => import(`./_id-Cj13oehp.js`), __vite__mapDeps([0, 1, 2, 3, 4, 5, 6, 7])),
    `component`,
  ),
});
export { i as t };
